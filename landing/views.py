...
def index(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Отправка email
            subject = f"Новая заявка с сайта от {form.cleaned_data['full_name']}"
            message = f"""
            ФИО: {form.cleaned_data['full_name']}
            Телефон: {form.cleaned_data['phone']}
            Email: {form.cleaned_data['email']}
            Организация: {form.cleaned_data['organization_name']}
            ...
            try:
                send_mail(
                    ...
                    [settings.CONTACT_FORM_RECIPIENT] # Добавьте этот email в settings.py
                )
                messages.success(request, 'Ваша заявка успешно отправлена!')
            except Exception as e:
                messages.error(request, f'Произошла ошибка при отправке заявки: {e}')
